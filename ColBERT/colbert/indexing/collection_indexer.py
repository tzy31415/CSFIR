...
class CollectionIndexer():
    ...
    def _sample_embeddings(self, sampled_pids):
        ...
        # print(type(local_pids))
        # print(len(local_pids))
        # print(local_pids[0])
        # assert 1==2
        # local_sample = [{pid:passage} for pid, passage in local_pids if pid in sampled_pids]
        local_all_sample = [[pid, passage] for pid, passage in local_pids]
        local_sample = []
        passage2pid = {}

        for i in local_all_sample:
            pid, passage = i
            if pid in sampled_pids:
                local_sample.append(passage)
                passage2pid[passage] = pid
                
        sampled_pids = [passage2pid[i] for i in local_sample]

        local_sample_embs, doclens = self.encoder.encode_passages(local_sample, sampled_pids)
    ...
    def index(self):
        ...
        local_pids = self.collection.enumerate(rank=self.rank)
        passage2pid = {}
        for pid, passage in local_pids:
             if pid in sampled_pids:
                 passage2pid[passage] = pid
        print(len(passage2pid))
        sampled_pids = []
        ...
        with self.saver.thread():
            ...
            for chunk_idx, offset, passages in tqdm.tqdm(batches, disable=self.rank > 0):
                ...
                # sampled_pids = [passage2pid[i] for i in passages]
                for idx, passage in enumerate(passages):
                    sampled_pids.append(offset + idx)
                
                embs, doclens = self.encoder.encode_passages(passages,sampled_pids) 
                if self.use_gpu:
